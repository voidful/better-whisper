...
class WhisperAttention(nn.Module):
    ...
    def forward(
        ...
        attn_weights = nn.functional.softmax(attn_weights/0.5, dim=-1)
...
class WhisperEncoder(WhisperPreTrainedModel):
    ...
    def _generate_attention_mask(self, input_length, batch_size, dtype, device):
        frames_per_second = 100  # 每秒的 frames 數量
        one_second_frames = frames_per_second * 1  # 設定 1 秒的窗口大小

        # 獲取最小值，用於無效位置的掩碼
        min_dtype = torch.finfo(dtype).min

        if input_length > one_second_frames:
            # 全注意力部分
            full_attention_mask = torch.full(
                (batch_size, 1, one_second_frames, one_second_frames),
                min_dtype,
                dtype=dtype,
                device=device
            )
            full_attention_mask = full_attention_mask.masked_fill(
                torch.tril(torch.ones_like(full_attention_mask), diagonal=0).bool(),
                0.0
            )

            # 因果注意力部分
            causal_attention_mask = torch.full(
                (batch_size, 1, input_length - one_second_frames, input_length - one_second_frames),
                min_dtype,
                dtype=dtype,
                device=device
            )
            causal_attention_mask = causal_attention_mask.masked_fill(
                torch.tril(torch.ones_like(causal_attention_mask), diagonal=0).bool(),
                0.0
            )

            # 拼接上半部分和下半部分
            upper_part = torch.cat(
                [
                    full_attention_mask,
                    torch.full(
                        (batch_size, 1, one_second_frames, input_length - one_second_frames),
                        min_dtype,
                        dtype=dtype,
                        device=device
                    ),
            ...
            lower_part = torch.cat(
                [
                    torch.full(
                        (batch_size, 1, input_length - one_second_frames, one_second_frames),
                        min_dtype,
                        dtype=dtype,
                        device=device
                    ),
            ...
            attention_mask = torch.cat([upper_part, lower_part], dim=2)
        else:
            # 短於 1 秒的輸入，使用全可見的 4D 注意力掩碼
            attention_mask = torch.zeros(
                (batch_size, 1, input_length, input_length),
                dtype=dtype,
                device=device
            )
            attention_mask = attention_mask.masked_fill(
                torch.tril(torch.ones_like(attention_mask), diagonal=0).bool(),
                0.0
            )
    ...
    def forward(
        ...
        embed_pos = self.embed_positions.weight  # coding=utf-8
...
class WhisperEncoder(WhisperPreTrainedModel):
    ...
    def _generate_attention_mask(self, input_length, batch_size, dtype, device):
        ...
        full_attention_mask = full_attention_mask.masked_fill(
            torch.tril(torch.ones_like(full_attention_mask, dtype=torch.bool), diagonal=0),
            0.0
        )
        causal_attention_mask = torch.full(
            (batch_size, 1, input_length - one_second_frames, input_length - one_second_frames),
        ...
        causal_attention_mask = causal_attention_mask.masked_fill(
            torch.tril(torch.ones_like(causal_attention_mask, dtype=torch.bool), diagonal=0),
            0.0
        )
        upper_part = torch.cat(
            [
...
class WhisperForAudioClassification(WhisperPreTrainedModel):
    ...
    def forward(
        ...
        if attention_mask is not None and attention_mask.dim() == 2:
            ...
            attention_mask = attention_mask.to(dtype=input_features.dtype)
